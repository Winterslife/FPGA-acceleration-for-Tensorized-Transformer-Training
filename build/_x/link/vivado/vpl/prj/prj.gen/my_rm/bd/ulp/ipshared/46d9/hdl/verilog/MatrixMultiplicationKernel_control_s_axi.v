...
// ==============================================================
`timescale 1ns/1ps
module MatrixMultiplicationKernel_control_s_axi
#(parameter
    ...
    output wire [63:0]                   b,
    output wire [63:0]                   c,
    output wire [31:0]                   size_n,
    output wire [31:0]                   size_k,
    output wire [31:0]                   size_m,
...
localparam
    ...
    ADDR_C_DATA_1      = 7'h2c,
    ADDR_C_CTRL        = 7'h30,
    ADDR_SIZE_N_DATA_0 = 7'h34,
    ADDR_SIZE_N_CTRL   = 7'h38,
    ADDR_SIZE_K_DATA_0 = 7'h3c,
    ADDR_SIZE_K_CTRL   = 7'h40,
    ADDR_SIZE_M_DATA_0 = 7'h44,
    ADDR_SIZE_M_CTRL   = 7'h48,
    WRIDLE             = 2'd0,
    WRDATA             = 2'd1,
    WRRESP             = 2'd2,
    WRRESET            = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_b = 'b0;
    reg  [63:0]                   int_c = 'b0;
    reg  [31:0]                   int_size_n = 'b0;
    reg  [31:0]                   int_size_k = 'b0;
    reg  [31:0]                   int_size_m = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_SIZE_N_DATA_0: begin
                    rdata <= int_size_n[31:0];
                end
                ADDR_SIZE_K_DATA_0: begin
                    rdata <= int_size_k[31:0];
                end
                ADDR_SIZE_M_DATA_0: begin
                    rdata <= int_size_m[31:0];
                end
            endcase
        end
    end
...
assign b             = int_b;
assign c             = int_c;
assign size_n        = int_size_n;
assign size_k        = int_size_k;
assign size_m        = int_size_m;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_SIZE_N_DATA_0)
            int_size_n[31:0] <= (WDATA[31:0] & wmask) | (int_size_n[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_SIZE_K_DATA_0)
            int_size_k[31:0] <= (WDATA[31:0] & wmask) | (int_size_k[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_SIZE_M_DATA_0)
            int_size_m[31:0] <= (WDATA[31:0] & wmask) | (int_size_m[31:0] & ~wmask);
    end
end
